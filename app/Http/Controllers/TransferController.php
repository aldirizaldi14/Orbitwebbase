...
class TransferController extends BaseController
{
    ...
    public function create(Request $request)
    {
        ...
        $data->transfer_created_by = $request->user()->user_username;
    ...
    public function update($id, Request $request)
    {
        ...
        $data->transfer_updated_by = $request->user()->user_username;
        $process = $this->save($request, $data);
        if ($process) {
            $response = ['status' => 'success', 'success' => true, 'message' => 'Save success'];
        } else {
    ...
    private function save(Request $request, TransferModel $data)
    {
        $data->transfer_code = $request->input('transfer_code');
        $data->transfer_time = $request->input('transfer_time');
        $data->transfer_warehouse_id = $request->input('transfer_warehouse_id');
        $process = $data->save();
    ...
    public function export(Request $request)
    {
        ...
        $sheet->setCellValue('A2', 'Transfer Data');
        ...
        foreach ($data as $sub) {
            ...
            $sheet->setCellValue('B'.$i, $sub->transfer_code);
            $sheet->setCellValue('C'.$i, $sub->transfer_time);
        }
