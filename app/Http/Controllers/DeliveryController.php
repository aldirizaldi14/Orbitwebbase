...
class DeliveryController extends BaseController
{
    ...
    public function detail(Request $request)
    {
        ...
        $data = DeliverydetModel::where('deliverydet_delivery_id', $delivery_id)
            ->join('product', 'product_id', 'deliverydet_product_id')
            ->get();
        $count = DeliverydetModel::where('deliverydet_delivery_id', $delivery_id)
            ->count();
    ...
    public function create(Request $request)
    {
        ...
        $data->delivery_created_by = $request->user()->user_username;
    ...
    public function update($id, Request $request)
    {
        ...
        $data->delivery_updated_by = $request->user()->user_username;
        $process = $this->save($request, $data);
        if ($process) {
            $response = ['status' => 'success', 'success' => true, 'message' => 'Save success'];
        } else {
    ...
    private function save(Request $request, DeliveryModel $data)
    {
        $data->delivery_code = $request->input('delivery_code');
        $data->delivery_time = $request->input('delivery_time');
        $data->delivery_warehouse_id = $request->input('delivery_warehouse_id');
        $process = $data->save();
    ...
    public function export(Request $request)
    {
        ...
        $sheet->setCellValue('A2', 'Delivery Data');
        ...
        foreach ($data as $sub) {
            ...
            $sheet->setCellValue('B'.$i, $sub->delivery_code);
            $sheet->setCellValue('C'.$i, $sub->delivery_time);
        }
