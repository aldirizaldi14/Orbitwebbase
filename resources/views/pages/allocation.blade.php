        ...
        <div class="modal-body">
            <div class="form-group">
                <div class="row">
                    ...
                    <div class="col-md-8">
                        <input type="text" class="form-control" name="allocation_time" readonly="">
                    </div>
                </div>
            </div>
            <div class="form-group">
                <div class="row">
                    ...
                    <div class="col-md-8">
                        <input type="text" class="form-control" name="allocation_code" readonly="">
                    </div>
                </div>
            </div>
            <div class="form-group">
                <div class="row">
                    ...
                    <div class="col-md-8">
                        <input type="text" class="form-control" name="allocation_product_id" readonly="">
                    </div>
                </div>
            </div>
            <div class="form-group">
                <div class="row">
                    <label class="col-md-4 control-label">Product *</label>
                    <div class="col-md-8">
                        <input type="text" class="form-control" name="allocation_product_desc" readonly="">
                    </div>
                </div>
            </div>
            <div class="table-responsive">
...
$(document).ready(function() {
    ...
    $("#btnAdd").click(function(){
        allocation_id='';
        $("input[name=allocation_code").val('');
        $("input[name=allocation_time").val('');
        $("input[name=allocation_product_id").val('');
        $("input[name=allocation_product_desc").val('');
        $('#modalForm').modal('show');
    });

    var tableColumn = [
        { data: "allocation_id", width : 50, sortable: false},
        { data: "allocation_time" },
        { data: "allocation_code" },
        { data: "product_code" },
        { data: "product_description" },
        { data: "allocation_id", width: 100, sortable: false}
    ];
    var orderSort = '';
    var orderDir = '';
    var table = $("#tableData").DataTable({
        ...
        drawCallback: function(e,response){
            $(".btn-edit").click(function(event){
                ...
                $("input[name=allocation_code]").val(data.allocation_code);
                $("input[name=allocation_time]").val(data.allocation_time);
                $("input[name=allocation_product_id]").val(data.product_code);
                $("input[name=allocation_product_desc]").val(data.product_description);
            ...
            $(".btn-delete").click(function(event){
                ...
                swal.fire({
                    title: "Delete "+ data.allocation_code +" ?",
                    type: "question",
                ...
                .then((confirm) => {
                    if (confirm.value) {
                        $.ajax({
                            url: '{{ url('allocation') }}/' + data.allocation_id,
                            method: "DELETE",
                            dataType : 'json'
                        })
                        .done(function(resp) {
    ...
    var tableColumnDetail = [
        { data: "allocationdet_id", width : 50, sortable: false},
        { data: "area_name" },
        { data: "allocationdet_qty" },
    ];
    ...
    $("#allocation_warehouse_id").select2({
        theme:'bootstrap',
        ajax: {
    ...
    $("#formData").submit(function (e)
    {
        ...
        $.ajax({
            url: '{{ url('allocation') }}' + (allocation_id ? '/' + allocation_id : ''),
            method: "POST",
            data: postData,
            processData: false,
            cache: false,
