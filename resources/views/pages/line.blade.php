...
$(document).ready(function() {
    ...
    $("#btnAdd").click(function(){
        ...
        $("input[name=line_description").val('');
        $('#modalForm').modal('show');
    });

    var tableColumn = [
        { data: "line_id", width : 50, sortable: false},
        { data: "line_name" },
        { data: "line_description" },
        { data: "line_id", width: 100, sortable: false}
    ];
    var orderSort = '';
    var orderDir = '';
    var table = $("#tableData").DataTable({
        ...
        drawCallback: function(e,response){
            $(".btn-edit").click(function(event){
                ...
                $("input[name=line_description]").val(data.line_description);
                $('#modalForm').modal('show');
            });
            $(".btn-delete").click(function(event){
                ...
                .then((confirm) => {
                    if (confirm.value) {
                        $.ajax({
                            url: '{{ url('line') }}/delete/' + data.line_id,
                            method: "post",
                            dataType : 'json'
                        })
                        .done(function(resp) {
